...
func select_pawn_to_attack(ctrl: TacticsControls) -> void:
	...
	if participant.attackable_pawn != null and participant.curr_pawn.stats.move_slots.size() > 0:
		var move_index: int = participant.curr_pawn.res.selected_move_index
		var move: PokemonMoveResource = participant.curr_pawn.stats.move_slots[move_index] if move_index >= 0 and move_index < participant.curr_pawn.stats.move_slots.size() else null
